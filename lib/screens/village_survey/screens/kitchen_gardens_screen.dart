  ...
  Widget _buildGardensContent() {
    return Column(
      children: [
        ...
        // Summary Card
        if (gardensController.text.isNotEmpty)
          SummaryCard(
            title: 'Kitchen Gardens Summary',
            items: [
              {'label': 'Kitchen Gardens:', 'value': gardensController.text},
              if ((int.tryParse(gardensController.text) ?? 0) > 0)
                {'label': 'Status:', 'value': 'Family nutrition supported'},
              if ((int.tryParse(gardensController.text) ?? 0) == 0)
                {'label': 'Status:', 'value': 'No kitchen gardens recorded'},
            ],
          ),
      ],
    );

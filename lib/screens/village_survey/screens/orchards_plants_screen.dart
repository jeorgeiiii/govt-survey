  ...
  Widget _buildOrchardsContent() {
    return Column(
      children: [
        ...
        // Total Display
        CalculatedDisplay(
          label: 'Total Orchards & Plants',
          value: _totalPlants,
          description: 'Sum of orchards and plants',
          icon: Icons.forest,
          color: Color(0xFF228B22), // Green color for plants
        ),
        
        SizedBox(height: 20),
        
        // Summary Card
        if (orchardsController.text.isNotEmpty || plantsController.text.isNotEmpty)
          SummaryCard(
            title: 'Orchards & Plants Summary',
            items: [
              if (orchardsController.text.isNotEmpty) 
                {'label': 'Orchards:', 'value': orchardsController.text},
              if (plantsController.text.isNotEmpty) 
                {'label': 'Plants:', 'value': plantsController.text},
              {'label': 'Total:', 'value': _totalPlants},
            ],
          ),
      ],
    );
